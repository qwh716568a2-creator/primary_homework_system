...
import type { ParentMessageItem } from '@/types/parent-portal'
import { parentMessageKindMap } from '@/utils/parent-portal-view'

const store = useParentPortalStore()
const activeFilter = ref<'all' | 'unread'>('all')
const selectedMessage = ref<ParentMessageItem | null>(null)
...
async function openMessage(item: ParentMessageItem) {
...
<template>
  <div class="parent-message-page">
    ...
    <section class="message-toolbar surface-card">
      <button
        type="button"
    ...
    <section v-if="filteredMessages.length" class="message-list surface-card">
      <article
      ...
      >
        ...
        <div class="message-card__content">
          <div class="message-card__meta">
            <span>{{ parentMessageKindMap[item.kind] || item.kind || '消息通知' }}</span>
            <span>{{ item.time }}</span>
            <span v-if="item.childName">{{ item.childName }}</span>
          </div>
          <h3>{{ item.title }}</h3>
          <p>{{ item.content }}</p>
        </div>
    ...
    <section v-else class="message-empty surface-card">
    ...
    <el-dialog v-model="detailVisible" title="消息详情" width="560px" class="message-detail-dialog">
      <div v-if="selectedMessage" class="message-detail">
        <div class="message-detail__meta">
          <span>{{ parentMessageKindMap[selectedMessage.kind] || selectedMessage.kind || '消息通知' }}</span>
          <span>{{ selectedMessage.time }}</span>
          <span v-if="selectedMessage.childName">{{ selectedMessage.childName }}</span>
