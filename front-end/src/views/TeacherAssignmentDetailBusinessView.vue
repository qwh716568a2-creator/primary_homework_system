...
const deleteRule = computed(() => getHomeworkDeleteRuleByDetail(detail.value))
...
async function remind(classId?: number | string) {
  try {
    ...
    ElMessage.success(classId ? '已发送本班催交通知' : '已发送作业催交通知')
  } catch (error) {
...
async function removeHomework() {
  ...
  if (!deleteRule.value.canDelete) {
    ElMessage.warning(deleteRule.value.tip)
    return
  }

  const confirmed = await ElMessageBox.confirm(
    `确认删除“${detail.value.baseInfo.title}”吗？删除后将无法恢复。`,
    '删除作业',
    {
...
<template>
  <div v-if="detail" class="page-stack">
    <div class="surface-card section-card mb-4" style="padding: 1.5rem">
      <el-page-header @back="router.back()" title="返回列表">
        ...
        <template #extra>
          <div class="header-actions">
            ...
            <el-button
              type="primary"
              :disabled="detail.baseInfo.status !== 'published'"
              @click="router.push(`/assignments/${detail.baseInfo.homeworkId}/grading`)"
            >
              进入批改
            </el-button>
            <el-button
            ...
            <el-button v-if="deleteRule.canDelete" type="danger" plain @click="removeHomework">
              删除
            </el-button>
          </div>
        </template>
        ...
        <el-descriptions :column="4" size="default" class="mt-4" border>
          <el-descriptions-item label="截止时间" label-align="right" align="center">
            {{ formatFullDateTime(detail.baseInfo.deadlineAt) }}
          </el-descriptions-item>
          <el-descriptions-item label="提交方式" label-align="right" align="center">
            {{ submitTypesText }}
          </el-descriptions-item>
          <el-descriptions-item label="逾期提交" label-align="right" align="center">
            <el-tag size="small" :type="detail.baseInfo.allowLateSubmit ? 'warning' : 'info'">
              {{ detail.baseInfo.allowLateSubmit ? '允许' : '不允许' }}
            </el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="重复提交" label-align="right" align="center">
            {{ detail.baseInfo.allowResubmit ? '允许' : '仅一次' }}
          </el-descriptions-item>
          <el-descriptions-item label="家长确认" label-align="right" align="center">
            {{ detail.baseInfo.needParentConfirm ? '需要' : '不需要' }}
          </el-descriptions-item>
        </el-descriptions>
      </el-page-header>
      ...
      <el-alert
        class="mt-4"
        :type="deleteRule.canDelete ? 'success' : 'warning'"
        :title="deleteRule.tip"
        :closable="false"
        show-icon
      />
    </div>
          ...
          <template v-if="detail.attachments.length">
            ...
            <div class="attachment-list">
              ...
              >
                ...
                <el-link type="primary" :href="resolveAttachmentUrl(item)" target="_blank" :download="getAttachmentName(item)" :underline="false">
                  下载
                </el-link>
              </div>
            </div>
      ...
      <el-col :span="12">
        <div class="surface-card section-card h-full">
          <div class="card-header pb-4">
            ...
            <span class="card-subtitle">支持按班级查看提交进度并催交</span>
          </div>
    ...
    <div class="surface-card section-card">
      ...
      >
        ...
        <el-table-column label="是否逾期" min-width="90" align="center">
          <template #default="{ row }">
            <el-tag :type="row.isLate ? 'danger' : 'info'" size="small">
              {{ row.isLate ? '是' : '否' }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" min-width="100" fixed="right" align="center">
          <template #default="{ row }">
            <el-button
              ...
              type="primary"
              plain
              @click="router.push(`/assignments/${detail.baseInfo.homeworkId}/grading?task=${row.taskId}`)"
            >
              去批改
            </el-button>
          </template>
