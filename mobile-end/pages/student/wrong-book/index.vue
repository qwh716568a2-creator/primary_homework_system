...
function goNew() {
  uni.navigateTo({
    url: '/pages/student/wrong-book/new'
  })
}

function openDetail(id: string) {
  uni.navigateTo({
    url: `/pages/student/wrong-book/detail?id=${id}`
  })
}
...
<template>
  <view class="mobile-page">
    <view class="detail-header">
      ...
      <view class="detail-header__back" @tap="goNew">新增</view>
    </view>

    <view class="wrongbook-hero shell-card">
      <text class="glass-chip">错题管理</text>
      <text class="wrongbook-hero__title">把做错的题整理成自己的提分清单</text>
      <text class="wrongbook-hero__meta">老师标记和自己添加的错题都会汇总在这里。</text>
    </view>
      ...
      <template v-else>
        ...
        >
          <view class="wrongbook-card__top">
            ...
            <text class="wrongbook-card__source">{{ item.sourceType === 'teacher_mark' ? '老师标记' : '我的整理' }}</text>
          </view>
