...
async function loadMessages() {
  ...
  try {
    await mobilePortalStore.loadParentMessages(readStatus.value)
  } catch (error) {
...
<template>
  <view class="mobile-page">
    <view>
      <text class="glass-chip">家长消息</text>
      <text class="messages-title">孩子作业提醒与批改反馈</text>
      <text class="messages-copy">这里集中展示新作业、催交通知和老师批改结果。</text>
    </view>
    ...
    <view class="messages-list">
      <view v-if="loading" class="shell-card empty-state">
        ...
        <text class="empty-state__copy">孩子的作业提醒马上就会显示出来。</text>
      </view>
      ...
      <template v-else>
        <view v-for="item in mobilePortalStore.parentMessages" :key="item.id" class="shell-card message-item">
          ...
          <text class="message-item__time">{{ item.childName ? `${item.childName} · ` : '' }}{{ item.time }}</text>
        </view>

        <view v-if="!mobilePortalStore.parentMessages.length" class="shell-card empty-state">
          <text class="empty-state__title">当前没有消息</text>
          <text class="empty-state__copy">孩子的新作业和老师反馈会显示在这里。</text>
        </view>
      </template>
    </view>

    <AppTabBar role="parent" active="messages" />
  </view>
</template>
