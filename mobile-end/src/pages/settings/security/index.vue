...
async function saveSecuritySettings() {
  try {
    await preferencesStore.saveSecuritySettings({
    ...
    uni.showToast({
      title: error instanceof Error ? error.message : '安全设置保存失败',
      icon: 'none'
    })
  }
}
...
async function resetSecuritySettings() {
  try {
    await preferencesStore.resetSecuritySettings()
    syncDraft()
    uni.showToast({ title: '已恢复默认安全设置', icon: 'success' })
  } catch (error) {
    uni.showToast({
      title: error instanceof Error ? error.message : '恢复默认安全设置失败',
      icon: 'none'
    })
  }
}
...
async function savePasswordCheck() {
  ...
  try {
    await preferencesStore.markPasswordChecked({
    ...
    uni.showToast({
      title: error instanceof Error ? error.message : '密码校验失败',
      icon: 'none'
    })
  }
}
