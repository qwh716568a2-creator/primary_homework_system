...
SET @drop_old_idx_sql = (
  SELECT CASE
    ...
    THEN 'ALTER TABLE `student_profile` DROP INDEX `uk_student_class_no`'
    ELSE 'SELECT 1'
  END
);
...
SET @add_new_idx_sql = (
  SELECT CASE
    ...
    THEN 'SELECT 1'
    ELSE 'ALTER TABLE `student_profile` ADD UNIQUE KEY `uk_student_school_no` (`school_id`, `student_no`)'
  END
);
